...
interface PinData {
  ...
  rank: number;
  /** Area price tier (zone floor applied) — unknown EV prices stay 'mid' */
  tier: PriceTier;
}
...
function pinData(app: AppStore): {
  ...
  if (app.mode === 'ev') {
    ...
    const pins = selectMapCharge(app).map((s) => ({
      ...
      tier:
        s.price != null
          ? priceTier(s.price.value, stats, s.searchKm <= app.radius)
          : ('mid' as const),
    }));
    return {
      pins,
  ...
  const pins = selectMapStations(app).map((s) => ({
    ...
    tier: priceTier(s.prices[app.fuel]!.value, stats, s.searchKm <= app.radius),
  }));
  return {
    pins,
...
export default function MapCanvas({ bottomInset = 0 }: { bottomInset?: number }) {
  ...
  useEffect(() => {
    ...
    for (const s of pins) {
      ...
      const sig = `${s.label}|${tier}|${best}|${focused}|${dot}`;
      wanted.add(s.id);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [app.stations.data, app.charge.data, app.mode, app.fuel, app.radius, app.brandSel, app.serviceTags, app.connSel, app.minPowerKw, app.evFreeOnly, app.evPricedOnly, app.userPos, app.searchPos, app.focusStationId, bottomInset]);
