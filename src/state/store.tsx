...
export function selectPriceStats(app: AppStore): PriceStats | null {
  ...
  return priceStatsOf(prices, zoneMin);
}

/** Same tiering over the charge domain — resolved €/kWh only (unknown excluded) */
export function selectChargePriceStats(app: AppStore): PriceStats | null {
  const prices = selectMapCharge(app)
    .map((s) => s.price?.value)
    .filter((v): v is number => v != null);
  let zoneMin = Infinity;
  for (const s of selectVisibleCharge(app)) {
    const p = s.price?.value;
    if (p != null && p < zoneMin) zoneMin = p;
  }
  return priceStatsOf(prices, zoneMin);
}

function priceStatsOf(prices: number[], zoneMin: number): PriceStats | null {
