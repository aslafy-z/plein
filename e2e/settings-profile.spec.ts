...
// The vehicle profile is not cosmetic: switching to « Motorcycle » applies its presets
// (E10, 15 L tank, 5.0 L/100 km) everywhere — the map chip, the prices
// compared, the savings math — and survives a reload.

test('the Motorcycle profile applies its fuel, tank and consumption presets', async ({ page }) => {
  await gotoMap(page)
  ...
  await page.reload()
  await expect(page.getByText('E10 ↻')).toBeVisible({ timeout: 15_000 })
  await page.getByText('Settings', { exact: true }).click()
  await expect(page.getByText('15 L', { exact: true })).toBeVisible()
})

test('changing the default fuel in Settings retargets the whole map', async ({ page }) => {
  await gotoMap(page)
  await page.getByText('Settings', { exact: true }).click()
